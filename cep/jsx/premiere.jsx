...
function sunInsertAsset(filePath, assetType) {
    try {
        switch (assetType) {
            ...
            case "transition":
            case "caption":
                // Check if it's actually a .mogrt file
                if (filePath.toLowerCase().indexOf(".mogrt") !== -1) {
            ...
            case "preset":
                // Presets get imported; applying is manual from Effects panel.
                var result = JSON.parse(sunImportFile(filePath));
                if (result.ok) {
                    result.note = "preset imported; apply it from the Effects panel";
                }
