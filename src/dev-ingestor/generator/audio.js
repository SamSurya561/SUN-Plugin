...
function buildCatalogue() {
  ...
  const whooshLengths = [
    { key: "Ultra Short", duration: 0.15 }, { key: "Short", duration: 0.42 },
    { key: "Medium", duration: 0.85 }, { key: "Long", duration: 1.6 },
    { key: "Extra Long", duration: 2.6 }, { key: "Cinematic", duration: 4.0 },
  ];
  const whooshMoves = [
    ...
    { key: "High Sweep", startFreq: 2000, endFreq: 8000, curve: 1.8 },
    { key: "Subtle", startFreq: 600, endFreq: 1200, curve: 1.0 },
    { key: "Aggressive", startFreq: 100, endFreq: 10000, curve: 3.0 },
  ];
  const whooshTones = [
  ...
  for (const len of whooshLengths) {
    for (const mv of whooshMoves) {
      for (const tn of whooshTones) {
        ...
        }, ["whoosh", mv.key.toLowerCase(), tn.key.toLowerCase()], 5);
      }
    }
  }
  ...
  for (const w of impactWeights) {
    for (const t of impactTones) {
      for (const s of impactSpaces) {
        ...
        }, ["impact", "hit", w.key.toLowerCase(), t.key.toLowerCase()], 5);
      }
    }
  }
  ...
  for (const k of glitchKinds) {
    for (const v of ["Light", "Heavy"]) {
      ...
      }, ["glitch", "digital", "error", k.key.toLowerCase()], 5);
    }
  }
  ...
  for (const len of transLengths) {
    for (const ch of transChars) {
      addTakes("transition", "transition", `Transition ${ch.key} ${len.key}`, {
        ...len, ...ch,
      }, ["transition", "whoosh", "impact", ch.key.toLowerCase()], 5);
    }
  }
  ...
  for (const k of foleyKinds) {
    addTakes("foley", "foley", `Foley ${k.key}`, k,
      ["foley", "organic", k.key.toLowerCase()], 5);
  }
